...
const Navbar = () => {
  ...
  const handleConnectMetamask = async () => {
    ...
    try {
      ...
      // There's a conditional block here that says if the user chainID isn't equal to that of the the Mumbai Network
      if (chainId !== "0x13881") {
        try {
  ...
  return (
    <nav className="flex justify-between items-center py-4 px-8 bg-[darkblue] text-white">
       <div>
      <a className="hover:text-gray-300 cursor-pointer">
        <Image src={OphirLogo} alt="VoteBlock logo" width={200} height={66} />
      </a>
    </div>

      <div>
        <button
          className="bg-[blueviolet] text-gray-300 py-2 px-4 rounded border-blueviolet"
